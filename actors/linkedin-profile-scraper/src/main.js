// linkedin-profile-scraper — RETIRED (fail-fast stub)
//
...
log.warning('linkedin-profile-scraper is retired and no longer returns data. LinkedIn now login-walls anonymous access to member profile pages, so a cookieless scraper cannot return profile data. No charges are applied.');

await Actor.pushData({
    retired: true,
    message: 'This actor has been retired. LinkedIn now login-walls anonymous access to member profile pages, so a cookieless scraper cannot return profile data. You have not been charged.',
