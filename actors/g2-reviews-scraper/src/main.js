// G2 Reviews Scraper — RETIRED (fail-fast stub)
//
// This actor is being retired. G2 blocks anonymous automated access (hard 403), which this actor cannot bypass, so the previous browser +
...
log.warning(
    'g2-reviews-scraper is retired and no longer returns data. G2 blocks anonymous automated access (hard 403), which this actor cannot bypass. No charges are applied.',
);

await Actor.pushData({
    retired: true,
    message:
        'This actor has been retired. G2 blocks anonymous automated access (hard 403), which this actor cannot bypass, so it cannot return data. You have not been charged.',
