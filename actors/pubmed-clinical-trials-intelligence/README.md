...
The first few rows per run are free so you can validate the schema before paying. After that, one charge per row regardless of which enrichment toggles are on. PubMed papers and clinical trial rows are charged at the same rate. iCite citation counts and MeSH terms are included at no extra per row charge.
