...
Export every Google review for any business into a clean JSON or CSV file. Get star ratings, full review text, reviewer name, review dates, photos, owner responses, and aggregate business ratings for your company and every competitor.

Built for local business operators, multi location brands, digital agencies, and BI teams who need Google review data without paying for a Google My Business reputation SaaS.
...
```mermaid
flowchart TD
    A[Local operators] -->|Catch bad reviews<br/>before they snowball| D[Google Review<br/>Data]
    B[Multi location chains] -->|Compare store performance<br/>across cities| D
    C[Digital agencies] -->|Client ready reports<br/>without Yext or Podium| D
    R[BI teams] -->|Feed sentiment models<br/>with real review text| D
...
| **Digital agency** | Client reputation reports pulled in minutes, not days |
| **BI analyst** | Clean JSON or CSV feed for sentiment and dashboards |
...
The actor opens each Google Maps business listing in a real browser, clicks the Reviews tab, applies your sort order, and scrolls the reviews panel until your cap is reached. Residential proxies keep you past Google's anti bot defenses.

---

## Quick start

Export 200 recent reviews for a single restaurant:
...
Compare 3 locations of a chain, pulling complaints only:
...
Or pull by business name plus city, no URL needed:
...
```json
{
  ...
  "rating": 5,
  "text": "Best anniversary dinner ever. The tomato tasting menu in summer is a highlight of the city.",
...
Every record carries both the review fields and the business rollup, so multi location pulls group cleanly in Excel, Sheets, or a BI tool.
...
|---|---|---|---|
| `placeUrls` | array | `[]` | Google Maps place URLs for businesses. |
| `placeUrl` | string | `null` | Single URL shortcut. Used when `placeUrls` is empty. |
...
**Can I export Google reviews without using the Places API?**
Yes. The official API charges per request and caps results at 5 reviews per place. This actor pulls the full review history from the public Maps UI for a few cents per run.

**How do I monitor my Google reviews without paying for Yext or Podium?**
Schedule this actor weekly against your Maps URL. Export the latest reviews and diff against last week in your own spreadsheet. Replaces a $300+ monthly subscription.

**Can I compare multiple business locations in one run?**
Yes. Pass every URL in `placeUrls`. Every record includes `businessName` and `businessAddress` for easy grouping.
...
Yes. Restaurants, hotels, retail, services, healthcare, any place with a Google Maps listing.

**How fresh is the data?**
Live at query time. Every run pulls straight from Google Maps.

**Can I search by business name instead of pasting a URL?**
Yes. Use `searchQueries` with "Business Name City". The actor resolves each query to the first Maps match.

**Why residential proxies?**
Google Maps is aggressive with anti bot defenses. Datacenter proxies get blocked within a few requests. Residential proxies keep runs clean, and the actor ships with residential defaults.
