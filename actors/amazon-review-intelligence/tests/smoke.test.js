...
test('actor.json parses and has required fields', () => {
    ...
    assert.equal(actorJson.name, 'amazon-review-intelligence');
...
test('input schema parses and has expected fields', () => {
    ...
    assert.ok(schema.properties.productUrls);
    assert.ok(schema.properties.maxReviews);
    assert.ok(schema.properties.sortBy);
    assert.ok(schema.properties.amazonDomain);
    assert.ok(schema.properties.filterByRating);
    assert.ok(schema.properties.verifiedOnly);
...
test('sortBy enum includes all expected values', () => {
    ...
    assert.ok(sortValues.includes('RECENT'));
    assert.ok(sortValues.includes('HELPFUL'));
});

test('amazonDomain enum includes major marketplaces', () => {
    const schema = JSON.parse(readFileSync(resolve(actorRoot, '.actor/input_schema.json'), 'utf8'));
    const domains = schema.properties.amazonDomain.enum;
    assert.ok(domains.includes('amazon.com'));
    assert.ok(domains.includes('amazon.co.uk'));
    assert.ok(domains.includes('amazon.de'));
    assert.ok(domains.includes('amazon.co.jp'));
});
...
test('maxReviews cap is enforced in code', () => {
    const src = readFileSync(resolve(actorRoot, 'src/main.js'), 'utf8');
    assert.match(src, /totalPushed/);
    assert.match(src, /maxReviews/);
});
...
test('ASIN extraction handles multiple URL formats', () => {
    const src = readFileSync(resolve(actorRoot, 'src/main.js'), 'utf8');
    assert.match(src, /\/dp\//);
    assert.match(src, /\/gp\/product\//);
    assert.match(src, /product-reviews/);
});
